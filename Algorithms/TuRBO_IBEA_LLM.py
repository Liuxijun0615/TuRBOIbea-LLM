# Algorithms/TuRBO_IBEA_LLM.py
from EA_Operators.TuRBO_LLM_EA import TuRBLLMEA
import pickle
import time
import copy
import numpy as np
...
def TuRBO_IBEA_LLM(problem, max_iter, pop_size, api_key, llm_model, save_path):
    # 参数设置
    initial_prompt = config.INITIAL_PROMPT
    example = config.EXAMPLE_PROMPT
    crossover_prompt = config.CROSSOVER_PROMPT_IBEA_NSGA2

    experiment_name = "TuRBO_IBEA_LLM_Experiment"
    algorithm_name = "TuRBO-IBEA-LLM"
    ...
    saver.save_metrics({'population_size': pop_size, 'algorithm': 'TuRBO'}, 'config')

    # 初始化
    global_start_time = time.time()
    print(f"🔬 TuRBO-IBEA-LLM 实验开始!")

    turbo_llm_ea = TuRBLLMEA(pop_size, initial_prompt, crossover_prompt, llm_model, api_key)
    ...
    for iter in range(max_iter):
        ...
        print(f"\n🔄 第 {iter + 1}/{max_iter} 代开始 (TuRBO)")

        # TuRBO 生成
        offspring, source_map = turbo_llm_ea.turbo_generation(pop, pop_size)
